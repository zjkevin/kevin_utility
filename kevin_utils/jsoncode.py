''' 参照http状态码，为防止和http状态码概念混淆，编码的起始位置做适当变动
    主要用于前端判断状态的依据
    1100-1199 用于指定客户端应相应的某些动作。 
    1200-1299 用于表示请求成功。 
    1300-1399 用于已经移动的文件并且常被包含在定位头信息中指定新的地址信息。 
    1400-1499 用于指出客户端的错误。 
    1500-1599 用于支持服务器错误。 
...
410 (Gone/已经不存在)
410 (SC_GONE)告诉客户端所请求的文档已经不存在并且没有更新的地址。410状态不同于404，410是在指导文档已被移走的情况下使用，
而404则用于未知原因的无法访问。该状态码是新加入 HTTP 1.1中的。 
...
class JSON_Code(object):
    ...
    RESPONSE_OK = 1200
    # 服务器超时
    SERVER_TIME_OUT = 1504
    # 请求资源不存在
    REQUEST_NOT_EXIST= 1404
    # 服务器错误
    SERVER_ERR = 1500